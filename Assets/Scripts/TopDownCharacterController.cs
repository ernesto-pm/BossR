...
public enum CharacterState
    ...
    public class TopDownCharacterController : MonoBehaviour, ICharacterController
    {
        ...
        private bool _blinkRequested = false;
        private Vector3 _blinkStartPosition;
        private Vector3 _blinkTargetPosition;
        private float _blinkTimer;
        private Vector3 _lastMoveInputVector; // Add this to store the last move direction
        ...
        public void SetInputs(ref PlayerCharacterInputs inputs)
        {
            ...
            switch (CurrentCharacterState)
            {
                case CharacterState.Default:
                    {
                        // Store the move input vector for blink direction
                        _lastMoveInputVector = new Vector3(moveInputVector.x, 0f, moveInputVector.z);
                        
                        // For Hotline Miami style, we want screen-space movement
                        // Forward is always world forward, right is always world right
                        _moveInputVector = new Vector3(moveInputVector.x, 0f, moveInputVector.z);
                
                        // Look direction from the input rotation (from mouse position)
                        _lookInputVector = Vector3.ProjectOnPlane(inputs.CameraRotation * Vector3.forward, Motor.CharacterUp).normalized;
        ...
        public void UpdateRotation(ref Quaternion currentRotation, float deltaTime)
        {
            switch (CurrentCharacterState)
            {
                case CharacterState.Default:
                    {
                        // For top-down mouse control, we always want to orient towards the look direction
